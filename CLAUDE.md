...
- **Exile suppresses the family-death chronicle entry.** `apply_verdict("exile", ...)` sets `alive=False` and stamps `last_action="exiled for <crime>"`. The daily death-sweep in `world_service.advance_one_day()` would normally emit a generic "X has died" family entry, but it filters anything whose cause contains "exiled" — the `record_trial_verdict()` justice entry already narrates the banishment, and a second "has died" line wrongly implies the villager was killed. "exiled" is also in `_DEATH_WORDS` so the safety-net `"died after: …"` prefix doesn't pollute the action log.
- **Rest HP regen scales with ATK.** `apply_rest` in `welfare.py` uses `hp_delta = max(rand_int(2,6), int(0.05 * v["atk"]))`. The flat 2–6 was load-bearing in early game but useless at high ATK where one combat WIN scratch already exceeds 30 HP. If you change this, also reconsider `resolve_combat()`'s win-scratch (`dmg_var * 0.10`, capped at 25% of current HP) — they're a balanced pair.

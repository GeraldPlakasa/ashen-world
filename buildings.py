...
def update_tax_policy(characters, bank: dict, log_it: bool = False):
    ...
    # You could log via king["last_action"] here if you want,
    # but last_action will be overwritten later by daily actions,
...
def apply_tax_on_income(v: dict, gross: int, bank: dict):
    ...
    Args:
        v     : villager dict (not strictly needed, but available if you
                later want to track per-villager tax paid).
        gross : income before tax (int)
        bank  : dict with 'tax_rate' and 'balance'

    Returns:
        { "net": net_income, "tax": tax_amount }
    """
    if gross <= 0:
...
def building_priority_weights(king: dict | None) -> dict:
    ...
    Mirrors the JS buildingPriorityWeights logic.
    """
